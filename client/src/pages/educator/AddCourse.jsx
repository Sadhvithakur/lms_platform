...
const AddCourse = () => {
    ...
    // create/revoke preview URL for selected image
    useEffect(() => {
        if (!image) {
            if (previewUrl) {
    ...
    const handleOpenAddLecture = (chapterId) => {
        ...
        setChapters(prev => prev.map(ch => {
            ...
            return {
                ...
                chapterContent: [...(ch.chapterContent || []), {
                    lectureId: uniqid(),
                    lectureTitle: lectureDetails.lectureTitle || 'Untitled',
    ...
    const handleToggleChapter = (chapterId) => {
        setChapters(prev => prev.map(ch => ch.id === chapterId ? { ...ch, collapsed: !ch.collapsed } : ch));
    }
    ...
    const handleDeleteLecture = (chapterId, lectureId) => {
        setChapters(prev => prev.map(ch => {
            ...
            return { ...ch, chapterContent: (ch.chapterContent || []).filter(l => l.lectureId !== lectureId) };
        }));
    }
    ...
    return (
        <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
            <form className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
                ...
                {/* Add more fields for chapters and lectures here */}
                <div>
                    {chapters.map((chapter, chapterIndex) => (
                        <div key={chapter.id || chapterIndex} className="bg-white border rounded-lg mb-4">
                            <div className="flex justify-between items-center p-4 border-b">
                                ...
                                <img onClick={() => handleDeleteChapter(chapter.id)} src={assets.cross_icon} alt="" className='cursor-pointer' />
                            </div>
                            {!chapter.collapsed && (
                                <div className="p-4 ">
                                    {(chapter.chapterContent || []).map((lecture, lectureIndex) => (
                                        <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                                            ...
                                            <img onClick={() => handleDeleteLecture(chapter.id, lecture.lectureId)} src={assets.cross_icon} alt="" className='cursor-pointer' />
                                        </div>
                                    ))}
                                    <div onClick={() => handleOpenAddLecture(chapter.id)} className='inline-flex bg-gray-100 p-2 rounded cursor-pointer mt-2'>+ Add Lecture</div>
                                </div>
                            )}
                        </div>
                    ))}
                    <div onClick={handleAddChapter} className='flex justify-center items-center bg-blue-100 p-2 rounded-lg
                    ...
                    {showPopup && (
                        ...
                        bg-gray-800 bg-opacity-50'>
                            ...
                            max-w-md'>
                                ...
                                <button onClick={handleAddLecture} type='button' className='w-full bg-blue-400 text-white px-4 py-2 rounded'> Add </button>
